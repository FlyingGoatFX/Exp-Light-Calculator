...
string cachedParam = "null";//store editParam mode to return to
...
//display settings:
//USER CONTROLLABLE PRECISION NOT YET IMPLEMENTED, W.I.P.
byte rndPlaceHP = 6;//round high precision decimal values to this place when writing to console
byte rndPlaceLP = 3;//round low precision decimal values to this place when writing to console
byte rndPlacePRINT = 6;//round decimal values to this place for printing to txt
byte roundCondition = 0;//was user changing precision settings before redraw? If true(1)(set after setting change, before auto redraw), go to editParam = "r" after redraw, then reset roundCondition to false(0)
//COLOR NOT YET IMPLEMENTED, W.I.P.
byte colorModeValIn = 0;//0 for normal white/gray/darkGray/black, 1 for warm yellow/DarkYellow/DarkRed/black
byte colorModeMax = 2;
string colorModeOut = "Default";
var colorBright = ConsoleColor.White;//switches to Yellow
var colorMid = ConsoleColor.Gray;//switches to DarkYellow
var colorDark = ConsoleColor.DarkGray;//switches to DarkRed
var colorWarning = ConsoleColor.Yellow;//switches to Cyan or magenta or smth idk
//'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
//irradiance:
byte irrUnitIn = 2;
...
{
    Console.ForegroundColor = colorDark;
    Console.SetCursorPosition(20, Console.CursorTop);
    Console.Write("Exposure-to-Light Field Calculator");
    Thread.Sleep(1);
...
{   
    ...
    Console.ForegroundColor = colorBright;
    Console.SetCursorPosition(19, Console.CursorTop);
    Console.Write("[Exposure-to-Light Field Calculator]");
    Thread.Sleep(1);
...
Console.WriteLine("v1.11.0 by Mak, FlyingGoatFX");
...
if(irrValIn != 0 && !double.IsNaN(irrValIn) && distValIn != 0 && !double.IsNaN(distValIn))//quick calculate and display lumensCalc if there's valid data, else NaN
    ...
    if(cachedParam!="null")//first check if there's a cached editParam to be deployed at loop restart--if so, proceed to ask permission to change editParam to cached mode.
    {
       if(editParam!="h")//unless one of these excluded editParams, change editParam to cachedParam and reset cache.  Params on exclusion list must themselves change editParam.
       {
        editParam=cachedParam;
        cachedParam="null";
        }
    }
    Console.ForegroundColor = colorMid;
    Console.Clear();
        ...
        Console.Write("        -  - ---==≡[Exposure-to-Light Field Calculator]≡==--- -  -  ");
    Console.ForegroundColor = colorDark;
    Console.WriteLine("v1.11.0 by Mak,");
    Console.SetCursorPosition(77, Console.CursorTop);
    Console.Write("FG/FX\r");
    ...
    Console.SetCursorPosition(60, Console.CursorTop);
    Console.WriteLine("[h] Color Mode:");
    switch(colorModeValIn)
        {
            case 0:
            colorModeOut = "Default";
            break;

            case 1:
            colorModeOut = "Warm Light";
            break;

            case 2:
            colorModeOut = "Dark Adapt";
            break;
        }
    Console.SetCursorPosition(73-colorModeOut.Length, Console.CursorTop);
    Console.ForegroundColor = colorDark;
    Console.Write("└ " + colorModeOut);
    Console.ForegroundColor = colorMid;
    ...
    {
    Console.WriteLine("IRRADIANCE: " + Math.Round(irrValIn, rndPlaceHP, MidpointRounding.AwayFromZero) + " " + irrUnitOut + " (" + Math.Round(altIrrVal, rndPlaceHP, MidpointRounding.AwayFromZero) + altIrrUnit + ")");
    }else{Console.WriteLine("IRRADIANCE: ");}

    Console.WriteLine("DISTANCE: " + Math.Round(distValIn, rndPlaceHP, MidpointRounding.AwayFromZero) + " " + distUnitOut + " (" + Math.Round(altDistVal, rndPlaceHP, MidpointRounding.AwayFromZero) + altDistUnit + ")");
    Console.Write("LUMENS REQUIRED: ");
    Console.ForegroundColor = colorBright;
    Console.WriteLine(Math.Round(lumensCalc, rndPlaceHP, MidpointRounding.AwayFromZero) + " Lumens");
    Console.ForegroundColor = colorMid;
    if(editParam!="e"&& editParam!="d" && editParam!="h" && editParam!="p")//if edit parameter not already decided at loop restart, use readline.
    {
...
while(editParam == "e")
    {   
        ...
        Console.SetCursorPosition(0, Console.CursorTop+8);//go to edit indicator line
        Console.WriteLine(editParam);//fill indicator and sweep to prompt line
        ...
        switch(irrUnitStringput)
        {
            ...
            case "h"://user is trying to edit color mode, trigger break out of irr loop
            Console.SetCursorPosition(0, Console.CursorTop-1);
            Console.Write("                                            ");
            Console.SetCursorPosition(0, Console.CursorTop-1);
            Console.Write("                                                                                          \r");
            cachedParam = editParam;
            editParam = "h";
            break;
        ...
        {
                bool irrValValid;
                string irrValStringput;
                do
                {
                Console.Write("                                                                   \r");
                Console.Write("Enter IRRADIANCE in " + irrUnitOut + ": ");
                irrValStringput = Console.ReadLine();
                irrValValid = Double.TryParse(irrValStringput, out _);
                Console.SetCursorPosition(0, Console.CursorTop-1);
            }
            while (!irrValValid);

            Console.SetCursorPosition(0, Console.CursorTop+1);
            irrValIn = Convert.ToDouble(irrValStringput);
        ...
        if(expCondition == 1)
        {
            ...
            switch(expEditStringput)
            {
                ...
                isoStringput = Console.ReadLine();
                isoInisValid = Double.TryParse(isoStringput, out _);
                Console.SetCursorPosition(0, Console.CursorTop-1);//set cursor back up one line after readline, to restart loop
                }while(!isoInisValid);
                Console.SetCursorPosition(0, Console.CursorTop+1);//undo cursor reset since we've broken out of loop
                isoIn = Convert.ToDouble(isoStringput);
                    //overwrite UI:
                Console.SetCursorPosition(0, Console.CursorTop-1);
                ...
                case "S":
                    //input:
                bool shutterInisValid;
                string shutterStringput;
                do{
                Console.Write("                                                            \r");
                Console.Write("Enter Exp Time, Denom of 1 sec:  1/");
                shutterStringput = Console.ReadLine();
                shutterInisValid = Double.TryParse(shutterStringput, out _);
                Console.SetCursorPosition(0, Console.CursorTop-1);
                }while(!shutterInisValid);
                Console.SetCursorPosition(0, Console.CursorTop+1);
                shutterIn = Convert.ToDouble(shutterStringput);
                    //overwrite UI:
                Console.SetCursorPosition(0, Console.CursorTop-1);
                Console.Write("                                       ");//clear input line
                Console.SetCursorPosition(12, Console.CursorTop-6);//Go to IRRADIANCE readout
                ...
                case "G":
                    //input:
                bool FnumInisValid;
                string FnumStringput;
                do{
                Console.Write("                                                            \r");
                Console.Write("Enter ƒ/stop:  ƒ/");
                FnumStringput = Console.ReadLine();
                FnumInisValid = Double.TryParse(FnumStringput, out _);
                Console.SetCursorPosition(0, Console.CursorTop-1);
                }while(!FnumInisValid);
                Console.SetCursorPosition(0, Console.CursorTop+1);
                FnumIn = Convert.ToDouble(FnumStringput);
                    //overwrite UI:
                Console.SetCursorPosition(0, Console.CursorTop-1);
                Console.Write("                                       ");//clear input line
                Console.SetCursorPosition(12, Console.CursorTop-6);//Go to IRRADIANCE readout
                ...
                case "+": 
                    //input:
                bool expStopsisValid;
                string expStopsStringput;
                do{ Console.Write("                                                            \r");
                    Console.Write("Rate ISO +x stops: x =         ");
                    Console.ForegroundColor = colorDark;
                    Console.Write("(Neg values yield brighter exposure lvls.  Zero(0) to skip)");
                    Console.ForegroundColor = colorMid;
                    Console.SetCursorPosition(23, Console.CursorTop);//set cursor back to "x=" field
                    expStopsStringput = Console.ReadLine();
                    expStopsisValid = Double.TryParse(expStopsStringput, out _);
                    Console.SetCursorPosition(0, Console.CursorTop-1);
                }while(!expStopsisValid);
                Console.SetCursorPosition(0, Console.CursorTop+1);
                expStops = Convert.ToDouble(expStopsStringput);
            ...
            {//based on base params of 100 ISO, 1/48s, f/4 => 160*sqrt(2)fc
                ...
                Console.Write(Math.Round(irrValIn, rndPlaceHP, MidpointRounding.AwayFromZero) + " Footcandles" + " (" + Math.Round(Math.Pow(1/0.3048, 2)*irrValIn, rndPlaceHP, MidpointRounding.AwayFromZero) + "Lux)");
        ...
        if(irrValIn != 0 && !double.IsNaN(irrValIn) && distValIn != 0 && !double.IsNaN(distValIn))
        {
            ...
            Console.SetCursorPosition(17, Console.CursorTop);
            Console.ForegroundColor = colorBright;
            Console.Write(Math.Round(lumensCalc, rndPlaceHP, MidpointRounding.AwayFromZero) + " Lumens");
            Console.ForegroundColor = colorMid;
            Console.SetCursorPosition(0, Console.CursorTop+2);
        }else{lumensCalc = 0.0/0.0;}
...
while(editParam == "d")
    {   Console.SetCursorPosition(50, Console.CursorTop-9);//go to option [t]
        ...
        {   
            bool distValInisValid;
            string distValStringput;
            do{
                Console.Write("                                                            \r");
                Console.Write("Enter DISTANCE, in " + distUnitOut + ": ");
                distValStringput = Console.ReadLine();
                distValInisValid = Double.TryParse(distValStringput, out _);
                Console.SetCursorPosition(0, Console.CursorTop-1);
            }while(!distValInisValid);
            Console.SetCursorPosition(0, Console.CursorTop+1);
            distValIn = Convert.ToDouble(distValStringput);
...
while (editParam == "h")
        {
            ...
            {
                case 0:
                colorBright = ConsoleColor.White;
                colorMid = ConsoleColor.Gray;
                colorDark = ConsoleColor.DarkGray;
                colorWarning = ConsoleColor.Yellow;
                break;

                case 1:
                colorBright = ConsoleColor.Yellow;
                colorMid = ConsoleColor.DarkYellow;
                colorDark = ConsoleColor.DarkGray;
                colorWarning = ConsoleColor.Red;
                break;

                case 2:
                colorBright = ConsoleColor.DarkYellow;
                colorMid = ConsoleColor.Red;
                colorDark = ConsoleColor.DarkRed;
...
while(editParam=="p" && dirConfirmation == 0)
    {   
        ...
        {   
            ...
            Console.SetCursorPosition(0, Console.CursorTop-1);
            Console.Write("                                                                                          \r");
            cachedParam = editParam;//cache "p", in this case
            editParam = "h";
            break;
    ...
    if(editParam == "p"&&dirConfirmation==1)
        {
            ...
            printOutput = new string[]{fileName,
                                   ...
                                   "   " + infoBoxLn2,
                                   "   " + infoBoxLn3,"-",
                                   "IRRADIANCE: " + Math.Round(irrValIn, rndPlacePRINT, MidpointRounding.AwayFromZero) + " " + irrUnitOut + " (" + Math.Round(altIrrVal, rndPlacePRINT, MidpointRounding.AwayFromZero) + altIrrUnit + ")",
                                   "DISTANCE: " + Math.Round(distValIn, rndPlacePRINT, MidpointRounding.AwayFromZero) + " " + distUnitOut + " (" + Math.Round(altDistVal, rndPlacePRINT, MidpointRounding.AwayFromZero) + altDistUnit + ")","══════════════════",
                                   "LUMENS REQUIRED: " + Math.Round(lumensCalc, rndPlacePRINT, MidpointRounding.AwayFromZero) + " " + "Lm","════════════════════════════════════════════════════════════════════",
                                   "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", 
                                   "                                 Generated using Exposure-to-Light Field Calculator v1.11.0",
